  ...
  if ($row = db_fetch($db, $result)) {
    ...
    if ($n > 0) {
      ...
      foreach ($total_arr as $xh => $each) {
        $h = @explode("|", trim($each));
        $text .= $GLOBALS['function_list_mode']($class_n, eval('return '.$link.';'), $h, $row['id'].'_'.(abs($xh)+1));
        //$text .= '<span class="class_w_'.$class_n.''.$_GET['current'].'"><a onmouseover="addC(this)" onclick="addM(this)" href="'.eval('return '.$link.';').'"'.($h[2] != '' ? ' class="'.$h[2].'"':'').'>'.$h[1].'</a>'.$h[5].'</span>';
      }

      $text .= get_page_foot2($n, $class_more, $p, 'PseudoXMLHTTP.php?xml_id=class_'.$_GET['id'].'&xml_a=1&xml_file=class_page.php&id='.$_GET['id'].'&current='.$_GET['current'].'&column_id='.$_GET['column_id'].'&class_id='.$_GET['class_id'].'&class_title='.urlencode($class_title).'&p=', 'addPFrame" onclick="addG(\''.$_GET['id'].'\', this);', $class_n.$_GET['current']);
