  ...
  <div class="column'.($web['area'][$_GET['column_id']][$_GET['class_id']][2]==1?' qiangdiao_class':'').'" name="class_title_'.$i.'" id="class_title_'.$i.'"'.($i==1?' style="margin-top:0;"':'').'><input type="hidden" name="class_title[]" />
    ...
    if ($n > 0) {
        if (!empty($class_more) && is_numeric($class_more) && $class_more < $n) {
          ...
          $text_class_more .= get_page_foot2($n, $class_more, 1, 'PseudoXMLHTTP.php?xml_id=class_'.$i.'&xml_a=1&xml_file=class_page.php&id='.$i.'&current=&column_id='.$_GET['column_id'].'&class_id='.$_GET['class_id'].'&class_title='.urlencode($class_title).'&detail_title='.urlencode($_GET['detail_title']).'&p=', 'addPFrame" onclick="addG(\''.$i.'\', this);', $class_n);
        }
        ...
        foreach ($total_arr as $xh => $each) {
          $h = @explode("|", trim($each));
          $text .= $GLOBALS['function_list_mode']($class_n, eval('return '.$link.';'), $h, $id.'_'.(abs($xh)+1));

          //$text .= '<span class="class_w_'.$class_n.'"><a onmouseover="addC(this)" onclick="addM(this)" href="'.eval('return '.$link.';').'"'.($h[2] != '' ? ' class="'.$h[2].'"':'').'>'.$h[1].'</a>'.$h[5].'</span>';
        }
            ...
            switch ($dside) {
              ...
              if ($result_ != false) {
                ...
                while ($row_ = db_fetch($db, $result_)) {
                  ...
                  $text_dside .= '<span><a href='.($dpy == $dpy_ ? '"#" onclick="return false"' : '"'.eval('return '.$link.';').'"').'>'.eval('return '.$link_img.';').'</a></span>';
                  unset($row_, $detail_title, $dpy_, $img_d);
                }
                unset($link, $link_img);
                $text_dside .= '</div></div>';
              }
                  ...
                  if ($class_title_c) {
                    ...
                    if ($result_ != false) {
                      while ($row_ = db_fetch($db, $result_)) {
                        ...
                        $text_dside .= '<span><a href='.($dpy == $dpy_ ? '"#" onclick="return false"' : '"'.eval('return '.$link.';').'"').'>'.eval('return '.$link_img.';').'</a></span>';
                        unset($row_, $class_title, $ct, $detail_title, $dpy_);
                      }
                    }
                    $result_ = NULL;
                  }
...
<style type="text/css">
<!--
.dside_left { width:'.$bodywidth.'px; line-height:29px; text-align:center; }
#detail_body { width:'.$bodywidth.'px; overflow:hidden; }
.dside_right { padding-left:7px; border-left:1px #D4D4D4 solid; font-size:12px; text-align:left; }
...
<style type="text/css">
<!--
.dside_left { padding-right:7px; border-right:1px #D4D4D4 solid; font-size:12px; text-align:left; }
.dside_left li a { color:#0033CC; font-family:宋体; white-space:nowrap; }
.dside_right { width:'.$bodywidth.'px; line-height:29px; text-align:center; }
#detail_body { width:'.$bodywidth.'px; overflow:hidden; }
-->
</style>
...
?>
<div id="for_menu"><?php
if (file_exists('writable/ad/ad_bottom.txt')) {
  @ include ('writable/ad/ad_bottom.txt');
}
?>
