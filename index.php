...
function get_j_0($http, $link, $column_id, $class_id, $class_name = '', $tcolor_key) {
  ...
  if ($http = trim($http)) {
    ...
    foreach ($http_arr as $k => $e) {
      ...
      if ($sitenm > 750 - 22 - 25) { //22余负.class li.class_title { margin-right:25px; }
        break;
      } else {
        $step ++;
        $text .= '<li><a onclick="addM(this)" href="'.@eval('return '.$link.';').'"'.($h[2] != '' ? ' class="'.$h[2].'"' : '').''.($h[4] != '' && $web['d_type'] != 1 ? ' title="'.html_back($h[4]).'" onmouseover="sSD(this,event);"' : '').'>'.$h[1].'</a>'.(!empty($h[5]) ? $h[5] : '').'</li>';
...
function get_j_1($http, $link, $column_id, $class_id, $class_name = '', $tcolor_key) {
  ...
  //$sitenm = 12 + strlenth($class_name) + 12 + 12 + strlenth('&#8230;') + 12;
  $sitenm = (int)strlenth($class_name.'更多»') + 48;
  $e_w = floor((750-22-(25-12)-(25-12)-$sitenm) / $web['kuz_align'][0]);
  $text .= '
<style>
...
';
    ...
    $text .= '
<style>
.mingz_align li { width:'.(floor((750-22) / abs($web['mingz_align'][0]))).'px; }
</style>
<script type="text/javascript">
if (rightWidth == 900) {
  document.write(\'<style>\
...
<!--
function getBodyW() {
  return parseInt(document.getElementById('head').offsetWidth)>=1170?900:750;
}
...
<script>
if (rightWidth == 900) {
 document.write(\'<style> .right .class li { margin:0 \'+(12+parseInt((900-750)/'.$li_max.'/2))+\'px !important; } </style>\');
}
</script>';
  }
}
...
<?php
@ include ('writable/extra/inc/code/360base.html');
if (file_exists('writable/extra/inc/code/360middle.html')) {
  @ include ('writable/extra/inc/code/360middle.html');
}
...
<?php
  if (file_exists('writable/extra/inc/code/360side.html')) {
    @include('writable/extra/inc/code/360side.html');
  }
?>
...
<div id="extra_bottom">
<?php
if (file_exists('writable/extra/inc/code/360bottom.html')) {
  @ include ('writable/extra/inc/code/360bottom.html');
}
?>
</div>
...
<script type="text/javascript">
if (typeof hao360.docWrite == 'function') {
  document.write = hao360.docWrite;
}
</script>

    <?php echo file_exists('writable/js/referrer_top.js') ? '<div class="bottom_in"><span class="bottom_title">最新点入</span><script> document.write(\'<sc\'+\'r\'+\'ipt language="javascript" src="writable/js/referrer_top.js?\'+new Date().getTime()+\'" type="text/javascript"></sc\'+\'r\'+\'ipt>\'); </script></div>' : ''; ?>
