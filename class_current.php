  ...
  <div class="column'.($web['area'][$_GET['column_id']][$_GET['class_id']][2]==1?' qiangdiao_class':'').'"'.($i==1?' style="margin-top:0;"':'').' name="class_title_'.$i.'" id="class_title_'.$i.'" style="border:none;'.($i==1?' margin-top:0;':'').'">
    ...
    if ($n > 0) {
        if (!empty($class_more) && is_numeric($class_more) && $class_more < $n) {
          ...
          $text_class_more .= get_page_foot2($n, $class_more, 1, 'PseudoXMLHTTP.php?xml_id=class_'.$i.'&xml_a=1&xml_file=class_page.php&id='.$i.'&current=_s&column_id='.$_GET['column_id'].'&class_id='.$_GET['class_id'].'&class_title='.urlencode($class_title).'&p=', 'addPFrame" onclick="addG(\''.$i.'\', this);', $class_n.'_s');
